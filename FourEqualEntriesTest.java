...
public class FourEqualEntriesTest {
...
/*
        ...
        for (int rows = 0; rows <= a.length - 1; rows++) {
            for (int cols = a[rows].length - 1; cols <= 0; cols--) {
                if (a[rows][cols] == a[rows + 1][cols - 1]) {  // as rows increase, cols decrease/move left
                    count++;
                    if (count == 4) {
                        return true;
                    }
        ...
        // NEW tests backward diagonal
        int colsLength = a[0].length;
        System.out.println("Column length is: " + colsLength);
        
        for (int cols = colsLength; cols <= 0; cols--) {
            for (int rows = 0; rows <= a.length - 1; rows++) {
                if (a[rows][cols] == a[rows + 1][cols - 1]) {
                    count++;
                    if (count == 4) {
